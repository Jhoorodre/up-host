...
class Backend(QObject):
    ...
    def updateConfig(self, config_data):
        ...
        try:
            ...
            if "rootFolder" in config_dict:
                ...
                if root_path_str and not root_path_str.startswith('/'):
            ...
            if "outputFolder" in config_dict:
                ...
                if output_path_str:
                    ...
                    # Ensure absolute path
                    if output_path_str and not output_path_str.startswith('/'):
            ...
            # Update selected host (save active host selection)
            if "selectedHost" in config_dict:
                selected_host = str(config_dict["selectedHost"]).strip()
                if selected_host in self._available_hosts:
                    self.config_manager.config.selected_host = selected_host
                    self.uploader_service.set_host(selected_host)
        ...
        try:
            ...
            for item in sorted(root_folder.iterdir()):
                if item.is_dir():
                    ...
                    # Try to load cover from JSON using flexible search
                    cover_url = ""
                    try:
                        ...
                        if root_json.exists():
                            json_file = root_json
                            logger.debug(f"Found JSON method 0 (root) for {item.name}: {json_file.name}")
                        
                        # 0.1. Also try glob search in root folder
                        if not json_file:
                            for file in root_folder.glob("*.json"):
                                if sanitize_filename(item.name, is_file=False, remove_accents=True) == sanitize_filename(file.stem, is_file=True, remove_accents=True):
                                    json_file = file
                                    logger.debug(f"Found JSON method 0.1 (root glob) for {item.name}: {json_file.name}")
                                    break
                        ...
                        # Add fuzzy search in output folder for refreshMangaList
                        if not json_file and output_folder.exists():
                            logger.debug(f"Trying fuzzy search in output folder for refreshMangaList: {item.name}")
                            for file in output_folder.glob("*.json"):
                                ...
                                if (sanitized_title in file_stem or 
                                    ...
                                    json_file = file
                                    logger.debug(f"Found JSON via fuzzy match in output folder for refreshMangaList: {json_file}")
                                    break
    ...
    def _loadMangaInfo(self, manga_title: str, folder_chapter_count: int):
        ...
        try:
            ...
            if root_json.exists():
                json_file = root_json
                logger.debug(f"Found JSON method 0 (root) for _loadMangaInfo {manga_title}: {json_file.name}")
            ...
            if not json_file:
                for file in root_folder.glob("*.json"):
                    if sanitize_filename(manga_title, is_file=False, remove_accents=True) == sanitize_filename(file.stem, is_file=True, remove_accents=True):
                        json_file = file
                        logger.debug(f"Found JSON method 0.1 (root glob) for _loadMangaInfo {manga_title}: {json_file.name}")
                        break
            ...
            # Add fuzzy search in output folder for _loadMangaInfo
            if not json_file and output_folder.exists():
                logger.debug(f"Trying fuzzy search in output folder for _loadMangaInfo: {manga_title}")
                for file in output_folder.glob("*.json"):
                    file_stem = sanitize_filename(file.stem, is_file=False, remove_accents=True)
                    if (sanitized_title in file_stem or 
                        file_stem in sanitized_title or
                        self._fuzzy_match_names(sanitized_title, file_stem)):
                        json_file = file
                        logger.debug(f"Found JSON via fuzzy match in output folder for _loadMangaInfo: {json_file}")
                        break
            ...
            if json_file and json_file.exists():
                ...
                # Extract group from first chapter
                group = ""
                ...
                self._manga_info = {
                    ...
                    "group": data.get("group", group),  # Try JSON group field first, then extracted group
    ...
    def _fuzzy_match_names(self, name1: str, name2: str, threshold: float = 0.6) -> bool:
        """Simple fuzzy matching for manga names"""
    ...
    def updateExistingMetadata(self, metadata):
        ...
        try:
            ...
            if root_json.exists():
                json_file = root_json
                logger.debug(f"Found JSON method 0 (root) for updateExistingMetadata {self._current_manga.title}: {json_file.name}")
            
            # 0.1. Also try glob search in root folder
            if not json_file:
                for file in root_folder.glob("*.json"):
                    if sanitize_filename(self._current_manga.title, is_file=False, remove_accents=True) == sanitize_filename(file.stem, is_file=True, remove_accents=True):
                        json_file = file
                        logger.debug(f"Found JSON method 0.1 (root glob) for updateExistingMetadata {self._current_manga.title}: {json_file.name}")
                        break
            ...
            if json_file and json_file.exists():
                ...
                # Update metadata fields (exclude "group" as it belongs in chapters structure only)
                ...
                # CRITICAL: DO NOT add "group" to root level - it belongs in chapters structure only
                ...
                try:
                    ...
                    # Create new metadata structure (exclude "group" as it belongs in chapters structure only)
                    new_data = {
